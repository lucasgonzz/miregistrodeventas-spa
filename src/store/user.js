...
export default {
	...
	actions: {
		getModels({ commit, state }) {
			...
			return axios.get(`/api/super-${generals.methods.routeString(state.model_name)}`)
			.then(res => {
		...
		delete({ commit, state }) {
			return axios.delete(`/api/super-${generals.methods.routeString(state.model_name)}/${state.delete.id}`)
			.then(() => {
				commit('delete')
				commit('setToShow')
			})
		...
		deleteImage({ commit, state }) {
			return axios.delete(`/api/super-${generals.methods.routeString(state.model_name)}/image/${state.delete_image.id}`)
			.then((res) => {
