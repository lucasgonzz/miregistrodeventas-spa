	...
	mutations: {
		...
		setModel(state, value) {
			if (value.model) {
				...
				state.model.permissions_id = []
				state.model.permissions.forEach(permission => {
					state.model.permissions_id.push(permission.id)
				})
			} else {
				let obj = {
					id: null
		...
		addPermissionId(state, permission) {
			console.log('llego esto')
			console.log(permission)
			let index = state.model.permissions_id.findIndex(id => {
			...
			if (index == -1) {
				...
				state.model.permissions_id.splice(index, 1)
			}
		},
